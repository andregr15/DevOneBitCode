...
export default {
  ...
  mounted() {
    this.$resource('/tags').get()
        .then(
          response => this.set_tags(response.body.tags),
          errors => M.toast({
  ...
  methods: {
    set_tags(tags) {
      ...
      tags.forEach(
        (tag) => autoCompleteTags[tag.title] = null
      );
      this.tags = tags;
    ...
    addTag(event) {
      let tagToAdd = this.tags.find(
        tag => { return tag.title == event.target.value }
      );
    ...
    removeTag(tag) {
      ...
      this.$resource('/portfolios{/portfolioId}/tags{/tagId}').remove(
        { portfolioId: this.portfolioId, tagId: tag.id }
      ).then(
