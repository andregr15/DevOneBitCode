...
<script>
  ...
  export default{
    ...
    created() {
      this.$resource('/portfolios{/portfolioId}/abouts')
          .get({ portfolioId: this.portfolioId })
          .then(
            response => this.about = response.body.resource,
            error => M.toast({
    ...
    methods: {
      submit() {
        this.$resource('/portfolios{/portfolioId}/abouts{/id}')
            .update({
